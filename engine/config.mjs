...
export function normalizeQDocConfig(root, userConfig = {}, configPath = path.join(root, "qdoc.config.mjs")) {
  const config = {
    ...
    designSystemDir: relativePathValue(userConfig.designSystemDir, DEFAULT_CONFIG.designSystemDir),
  ...
  config.paths = {
    ...
    designSystemDir: path.join(documentRoot, config.designSystemDir),
