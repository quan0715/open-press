...
const DEPLOY_SOURCE_CLASS = [
  ...
  "rounded-full border border-[color-mix(in_srgb,var(--op-workspace-success)_18%,transparent)] bg-[color-mix(in_srgb,var(--op-workspace-success)_7%,transparent)] px-[7px]",
  "text-[10px] font-semibold leading-none text-[var(--op-workspace-success)]",
].join(" ");
const DEPLOY_DETAILS_CLASS = [
  "grid gap-2 border-y border-[var(--op-workspace-border-muted)] py-3",
].join(" ");
const DEPLOY_ROW_CLASS = "grid min-w-0 grid-cols-[78px_minmax(0,1fr)] items-center gap-3";
const DEPLOY_TERM_CLASS = "m-0 min-w-0 text-[11px] leading-[1.35] text-[var(--op-workspace-text-muted)]";
const DEPLOY_VALUE_CLASS = "m-0 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-left text-[11px] leading-[1.35] text-[var(--op-workspace-text-soft)]";
const DEPLOY_LINK_CLASS = "text-[var(--op-workspace-text-soft)] no-underline hover:text-[var(--op-workspace-accent)] hover:underline hover:underline-offset-[3px]";
const DEPLOY_STATUS_CLASS = "openpress-deploy-dialog__status inline-flex items-center justify-start gap-1.5 text-[var(--op-workspace-text-muted)]";
...
const DEPLOY_STATUS_KIND_CLASS: Record<string, string> = {
  online: "text-[var(--op-workspace-success)]",
  dirty: "text-[var(--op-workspace-accent)]",
  deploying: "text-[var(--op-workspace-accent)]",
  failed: "text-[var(--op-workspace-danger)]",
};
const DEPLOY_MESSAGE_CLASS = "openpress-deploy-dialog__message m-0 border-l-2 border-[var(--op-workspace-danger)] py-1.5 pl-2.5 pr-0 text-xs leading-[1.45] text-[var(--op-workspace-danger)]";
