...
import { QDocPublicPage, QDocPublicViewer, type QDocPageInspector } from "../src/qdoc/publicPage";
import type { QDocDocument } from "../src/qdoc/types";

const documentFixture: QDocDocument = {
...
describe("findQDocInspectorTarget", () => {
  it("finds the nearest qdoc block id from nested click targets", () => {
    const container = document.createElement("div");
    container.innerHTML = '<article data-qdoc-block-id="b-intro-0"><span>Title</span></article>';
    const nested = container.querySelector("span");

    expect(findQDocInspectorTarget(nested)).toEqual({ blockId: "b-intro-0", placement: "block" });
    expect(findQDocInspectorTarget(container)).toBeNull();
  });

  it("treats an insertion target as a comment before the next block", () => {
    const container = document.createElement("div");
    container.innerHTML = '<button data-qdoc-insert-before-block-id="b-intro-0"><span>新增</span></button>';
    const nested = container.querySelector("span");

    expect(findQDocInspectorTarget(nested)).toEqual({ blockId: "b-intro-0", placement: "before" });
  });
});

describe("useQDocInspector", () => {
  it("persists inspector mode and selects React block metadata from clicks", () => {
    ...
    expect(window.localStorage.getItem("qdoc:test-inspector")).toBe("on");
...
describe("submitQDocInspectorComment", () => {
  ...
  it("rejects empty notes before posting", async () => {
    ...
    await expect(submitQDocInspectorComment({
      block: documentFixture.source?.blockMap?.["b-intro-0"] ?? null,
      note: " ",
      fetchImpl,
  ...
  it("posts comment intent and placement in the inspector hint", async () => {
    ...
    expect(JSON.parse(String(request.body))).toMatchObject({
      note: "在這裡補一段練習題",
      hint: "qdoc-react-inspector intent=add placement=before",
    });
  });
});

describe("formatQDocInspectorHint", () => {
  it("keeps inspector comment metadata compact and parseable", () => {
    expect(formatQDocInspectorHint({ intent: "delete", placement: "block" })).toBe(
      "qdoc-react-inspector intent=delete placement=block",
    );
  });
});
...
describe("comment list helpers", () => {
  it("fetches pending comments from the dev endpoint", async () => {
    ...
    const comments = await fetchQDocInspectorComments({ fetchImpl });

    expect(comments).toEqual([
      { id: "c-1", path: "document/chapters/01-intro/content/01-start.mdx", line: 3, note: "修改這段" },
    ]);
    expect(fetchImpl).toHaveBeenCalledWith("/__qdoc/comment", { method: "GET" });
  });
  ...
  it("clears one pending comment through the dev endpoint", async () => {
    ...
    const result = await clearQDocInspectorComment({ id: "c-1", fetchImpl });
...
describe("QDocPublicPage inspector delegation", () => {
  it("lets inspector clicks win over internal anchor navigation", () => {
    const handleClick: QDocPageInspector["handleClick"] = vi.fn((event) => {
        ...
        pages={[
          {
            ...
            title: "Page",
            pageNumber: 1,
            html: '<section class="reader-page"><a href="#target" data-qdoc-block-id="b-intro-0">Intro anchor</a></section>',
            anchors: ["target"],
          },
        ]}
        currentPageIndex={0}
...
describe("React pre-paginated reader metadata", () => {
  it("treats build-time paginated React documents as ready on first render", () => {
        ...
        pages={[
          {
            ...
            title: "Page",
            pageNumber: 1,
            html: '<section class="reader-page reader-page--report" data-page-kind="report"><h2 data-qdoc-block-id="b-intro-0">Intro</h2></section>',
          },
        ]}
        style={{}}
      />,
    ...
    expect(document.querySelector(".qdoc-reader-app")?.getAttribute("data-qdoc-pagination")).toBe("ready");
  });
});

function InspectorHarness() {
  const inspector = useQDocInspector(documentFixture, {
    enabled: true,
    storageKey: "qdoc:test-inspector",
  });
...
function useAnchorNavigationState(inspector: QDocInspectorState) {
