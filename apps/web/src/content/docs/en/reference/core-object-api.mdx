...
- Use stable `label` on `Text`, `Line`, and `MediaObject` in slide sources.
...
New slides should compose `Slide`, `Frame`, `Text`, media primitives, and style classes directly. Old compound wrappers such as `TitleSlide`, `TwoColumnSlide`, and `SlideProtocol` are not the default path for new workspaces.
