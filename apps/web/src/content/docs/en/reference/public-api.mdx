...
| `Slide` | Component | Slide page root container, implemented as a slide-friendly `Frame` wrapper. |
...
| `Text` | Component | Unstyled editable text object. Slide sources should provide a stable `label`; `label` is optional descriptive metadata. |
| `Line` | Component | Trackable line primitive for dividers, rules, timeline marks, and other fixed visual objects. |
| `MediaObject` | Component | Trackable media figure wrapper for images, video, or other media regions. |
| `Media` | Component | Media content element, usually nested inside `MediaObject`. |
| `MediaCaption` | Component | Caption element. Use `Text` inside it when the caption should be editable. |
| `ObjectEntity` | Component | Low-level editing anchor wrapper component. Developers should generally prefer `Text`, `Line`, `MediaObject`, or `Frame`. |
...
**Types:** `FixedBox`, `FrameLayout`, `FrameProps`, `SlideProps`, `SlideMeta`, `MdxAreaProps`, `MdxAreaOverflow`, `PressProps`, `WorkspaceProps`, `PageGeometry`, `PressSource`, `ObjectEntityProps`, `ObjectEntityElement`, `TextProps`, `LineProps`, `MediaObjectProps`, `MediaProps`, `MediaCaptionProps`, `BaseFigureProps`, `MediaFigureProps`

See [Core Object API](/docs/reference/core-object-api) for fixed-layout objects, direct slide sources, and editable object identity rules.

## `@open-press/core/theme`

Theme token module for slide and fixed-layout workspaces. It defines portable colors / fonts / typography tokens and emits CSS variables for `theme/default.css`.

| Export | Kind | Purpose |
| --- | --- | --- |
| `defineTheme(input)` | Function | Normalizes theme input into `DefinedOpenPressTheme`, including resolved colors, typography, and CSS variables. |
| `themeToCssVariables(theme)` | Function | Returns a CSS variable map that can be passed to React `style`. |
| `themeToCssText(theme, selector?)` | Function | Generates CSS text, useful for writing stylesheets or theme previews. |
| `ThemeColorSwatches` | Component | Renders theme color swatches. |
| `ThemeTypographyGraph` | Component | Renders the typography token graph. |
| `ThemeSpec` | Component | Renders a complete theme specimen. |
...
See [Theme API](/docs/reference/theme-api) for details.
...
- **Initialize project**: `npm create @open-press <target> -- --type <pages|slides> --title "..."`
- **Add Press**: `open-press create <slug> --type <pages|slides>`
- **Install skills**: `npx skills add <owner/repo>`
- **Dev operations**: `npm run dev`, `npm run build`, `npm run preview`, `npm run typecheck`, `npm run openpress:pdf`, `npm run openpress:word`
