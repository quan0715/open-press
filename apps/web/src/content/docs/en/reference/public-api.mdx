...
| `Text` | Component | Unstyled editable text object. Templates should provide a stable `label`; `label` is optional descriptive metadata. |
...
| `BaseCallout` | Component | Legacy base callout box. New templates should prefer composing `Frame` + `Text`. |
...
**Types:** `FixedBox`, `FrameLayout`, `FrameProps`, `SlideProps`, `SlideMeta`, `MdxAreaProps`, `MdxAreaOverflow`, `PressProps`, `WorkspaceProps`, `PageGeometry`, `PressSource`, `ObjectEntityProps`, `ObjectEntityElement`, `TextProps`, `LineProps`, `MediaObjectProps`, `MediaProps`, `MediaCaptionProps`, `BaseFigureProps`, `BaseCalloutProps`, `BaseCalloutKind`, `MediaFigureProps`

See [Core Object API](/docs/reference/core-object-api) for fixed-layout objects, slide templates, and editable object identity rules.
