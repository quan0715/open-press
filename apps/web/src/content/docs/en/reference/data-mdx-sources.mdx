---
title: "MDX Sources"
eyebrow: "@open-press/core/mdx"
description: "MDX source registrations are passed as a prop to <Press>. The engine reads sources to discover content files, compile blocks, and flow them into MdxArea slots — making them available for manuscript utilities, search/replace, and the comment marker writer."
---

The `mdxSource` interface provides the functionality to externally register MDX content directories. The system collects parse targets via this registry and allocates compiled block data to `MdxArea`, the search engine, and editing endpoints.

<div class="callout">
  <strong>1.0 Contract:</strong> Source declaration is a required prop for `<Press sources={...}>`. See the `<Press>` component reference.
</div>

<ApiEntry
  name="<Press sources>"
  kind="config"
  importFrom={`<Press sources={[\n  mdxSource({ id: "story", preset: "section-folders", root: "chapters" }),\n]}>`}
  summary="Declares multiple content data streams in `<Press>`, assigning each a globally unique `id` for consumption or addressing by other document components."
>
  ### Example: Multiple Data Source Registration

  ```tsx
  <Press
    title="Paper"
    page="a4"
    sources={[
      mdxSource({ id: "story", preset: "section-folders", root: "report/chapters" }),
      mdxSource({ id: "appendix", preset: "section-folders", root: "report/appendix" }),
    ]}
  >
    <Sections source="story" />
    <Sections source="appendix" />
  </Press>
  ```
</ApiEntry>

<ApiEntry
  name="mdxSource"
  kind="function"
  importFrom={'import { mdxSource } from "@open-press/core/mdx";'}
  signature={`mdxSource(options: MdxSourceOptions): SourceRegistration`}
  summary="Factory function returning a standardized data source registration object for internal processing by `<Press>`."
>
  <PropsTable
    title="Options (MdxSourceOptions)"
    rows={[
      {
        name: "id",
        type: "string",
        required: true,
        description: "Identifier. Depended upon by `<MdxArea chainId>`, `<Sections source>`, and search/replace CLI.",
      },
      {
        name: "preset",
        type: '"section-folders" | "file-list"',
        required: true,
        description: "`section-folders` scans using conventional directory structures; `file-list` scans relying on an explicitly provided `files` list.",
      },
      {
        name: "root",
        type: "string",
        description: "The search root relative to `press/` (e.g., `report/chapters`). Ignored when using `file-list` mode.",
      },
      {
        name: "files",
        type: "string[]",
        description: "An array of specific relative MDX file paths when `preset: \"file-list\"` is specified.",
      },
    ]}
  />

  ### Example: Conventional Directory Resolution (`section-folders`)

  ```text
  press/
    report/
      chapters/
        01-intro/
          content/
            01-overview.mdx
            02-context.mdx
  ```
  The system will concatenate and merge files within `01-intro/content/` in lexicographical order into a single chapter block stream.

  ### Example: Explicit File List (`file-list`)

  ```tsx
  mdxSource({
    id: "story",
    preset: "file-list",
    files: ["intro.mdx", "results.mdx", "conclusion.mdx"],
  });
  ```
</ApiEntry>

## Content Compilation Restrictions (Block-only Rule)

Source MDX must strictly follow the **Block-only** rule. Because the renderer needs to perform precise block measurement and pagination, JSX tags within MDX must be top-level block elements; inline mixed JSX declarations are strictly forbidden.

### Supported Syntax

```mdx
<TableCaption>Daily ridership</TableCaption>

| Day | Riders |
| --- | --- |
| Mon | 1,204 |
```

### Rejected Syntax (Will throw a compilation error)

```mdx
This is a piece of text containing <Highlight>inline tags</Highlight>.
```

## Figure and table cross-references

Give figures and tables stable IDs, then reference them in prose with `@fig-*` and `@tbl-*`. OpenPress replaces each token with the current caption label and links it to the target. Adding, removing, or reordering captions no longer requires rewriting hard-coded numbers.

```mdx
@fig-system-flow shows the main flow. Results appear in @tbl-results.

<figure id="fig-system-flow">
  <img src="media/system-flow.svg" alt="System flow" />
  <figcaption>Main system flow</figcaption>
</figure>

<TableCaption id="tbl-results">Evaluation results</TableCaption>

| Metric | Result |
| --- | --- |
| MAE | 4.89 |
```

Figure IDs must start with `fig-`; table IDs must start with `tbl-`. IDs must be unique within a Press, and export fails for missing or mismatched targets. Custom figure or table components must forward their `id` prop to the inner `<figure>` or `<table>` element.

## Dependencies and System Integration

Source registration directly unlocks the following system features:

- **Dynamic Rendering**: `<MdxArea>` interfaces with this source using `chainId`.
- **Auto-pagination**: `<Sections>` and `<Toc>` rely on the source's directory tree traversal to execute tasks.
- **Global Operations**: CLI `search` and `replace` only operate on file sets registered via `mdxSource`.
- **Markup System**: The Workbench's `source-edit` endpoint and `@openpress-comment` marker writer target file modification locations based on the source map.

Resource directories such as components (`components/`), media (`media/`), and theme (`theme/`) do not involve the pagination rendering mechanism, and are therefore not within the registration scope of `sources`, but are handled by other `<Press>` properties or conventions.
