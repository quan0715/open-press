...
<h2>The working contract</h2>
  ...
  <h3>Example: Install skills first</h3>
...
```
  ...
  <p>
    For CLI-first slide work, run create directly. The create package bootstraps the workspace and installs OpenPress
    packages; skills still own opinionated starter content and page-based artifact structure.
  </p>

  <h3>Example: CLI-first start</h3>

```bash
npm create @open-press@latest my-deck -- --type slides --title "Transport models"
cd my-deck
npm run dev
```
