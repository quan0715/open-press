...
| `Slide` | 元件 | 簡報頁面根容器，是 slide-friendly 的 `Frame` wrapper。 |
...
| `Text` | 元件 | 可被追蹤與行內編輯之無樣式文字物件。Template 應提供穩定 `label`；`label` 為選用描述。 |
| `Line` | 元件 | 可追蹤線條 primitive，用於 divider、rule、timeline mark 等固定視覺物件。 |
| `MediaObject` | 元件 | 可追蹤媒體 figure wrapper，用於圖片、影片或其他媒體區塊。 |
| `Media` | 元件 | 媒體內容元素，通常放在 `MediaObject` 內。 |
| `MediaCaption` | 元件 | 圖說元素；需要可編輯圖說時，內部可組合 `Text`。 |
| `ObjectEntity` | 元件 | 底層編輯錨點包裹元件，開發者通常應優先使用 `Text`、`Line`、`MediaObject` 或 `Frame`。 |
...
**型別定義 (Types):** `FixedBox`, `FrameLayout`, `FrameProps`, `SlideProps`, `SlideMeta`, `MdxAreaProps`, `MdxAreaOverflow`, `PressProps`, `WorkspaceProps`, `PageGeometry`, `PressSource`, `ObjectEntityProps`, `ObjectEntityElement`, `TextProps`, `LineProps`, `MediaObjectProps`, `MediaProps`, `MediaCaptionProps`, `BaseFigureProps`, `MediaFigureProps`

固定版面、簡報 template 與可編輯物件的使用方式請參考 [Core Object API](/docs/reference/core-object-api)。

## `@open-press/core/theme`

Slide 與 fixed-layout workspace 的 theme token 模組。它定義可移植的 colors / fonts / typography token，並輸出 CSS variables 供 `theme/default.css` 使用。

| 匯出 (Export) | 種類 (Kind) | 用途 (Purpose) |
| --- | --- | --- |
| `defineTheme(input)` | 函數 | 將 theme input 正規化為 `DefinedOpenPressTheme`，包含 resolved colors、typography 與 CSS variables。 |
| `themeToCssVariables(theme)` | 函數 | 回傳可放入 React `style` 的 CSS variable map。 |
| `themeToCssText(theme, selector?)` | 函數 | 產生 CSS text，適合寫入 stylesheet 或 theme preview。 |
| `ThemeColorSwatches` | 元件 | 渲染 theme color swatches。 |
| `ThemeTypographyGraph` | 元件 | 渲染 typography token graph。 |
| `ThemeSpec` | 元件 | 渲染完整 theme specimen。 |

**型別定義 (Types):** `OpenPressThemeInput`, `ThemeColorInput`, `ThemeTypographyInput`, `DefinedOpenPressTheme`, `DefinedThemeColor`, `DefinedThemeTypography`, `ThemeVisualizationProps`

詳細使用方式請參考 [Theme API](/docs/reference/theme-api)。
...
- **初始化專案**：`npm create @open-press <target> -- --type <pages|slides> --title "..."`
- **新增 Press**：`open-press create <slug> --type <pages|slides>`
- **安裝技能**：`npx skills add <owner/repo>`
- **開發操作**：`npm run dev`, `npm run build`, `npm run preview`, `npm run typecheck`, `npm run openpress:pdf`, `npm run openpress:word`
...
| `/__openpress/local-word-export` | POST | 在本地端構建 page Press 的 Word .docx 匯出檔案。 |
| `/__openpress/local-word-file` | GET | 回傳建置完成之本地 Word .docx 檔案實體。 |
