...
<h2>穩定性承諾 (Stability promise)</h2>
  ...
  <p>
    本頁面上的介面受到 semver 語意化版本控制的保護：對任何列出的匯出 (export)、設定欄位、CSS 變數、標記格式或是開發端點進行破壞性變更 (breaking change)，都需要進行主要版本的更新 (major version bump)。在此清單之外的內部標籤與模組 (任何只能透過深層匯入觸及的東西)，都可能在任何次要版本更新中發生變更，而且不會在 CHANGELOG 內註明這可能會破壞您下游的程式碼。
  </p>
  ...
  <p>
    <code>@open-press/create</code> 用來建立 (bootstrap) 一個全新且遵循資料夾慣例的 workspace。package bootstrapper 支援 <code>--type slides</code>；頁面型結構由 OpenPress skills 負責建立。
...
open-press create appendix --type slides</code></pre>
  ...
  <p>以下的東西雖然可觸及，但被明確標示為 <strong>不</strong> 屬於公開 API：</p>
