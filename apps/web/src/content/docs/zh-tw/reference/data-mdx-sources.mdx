---
title: "MDX 來源"
eyebrow: "@open-press/core/mdx"
description: "MDX 來源註冊會作為一個 prop 傳遞給 <Press>。引擎讀取來源以發現內容檔案、編譯區塊，並將它們送入 MdxArea 插槽 — 進而提供給文稿輔助工具、搜尋/替換以及註解標記寫入器使用。"
---

`mdxSource` 介面提供對外註冊 MDX 內容目錄的功能。系統透過此註冊表收集解析目標，並將編譯後之區塊資料分配給 `MdxArea`、搜尋引擎及編輯端點。

<div class="callout">
  <strong>1.0 契約：</strong> 來源聲明為 `<Press sources={...}>` 的必填屬性。參見 `<Press>` 元件參考。
</div>

<ApiEntry
  name="<Press sources>"
  kind="config"
  importFrom={`<Press sources={[\n  mdxSource({ id: "story", preset: "section-folders", root: "chapters" }),\n]}>`}
  summary="於 `<Press>` 宣告多個內容資料流，賦予各資料流全域唯一 `id`，供其他文件元件消費或定址。"
>
  ### 範例：多資料來源註冊

  ```tsx
  <Press
    title="Paper"
    page="a4"
    sources={[
      mdxSource({ id: "story", preset: "section-folders", root: "report/chapters" }),
      mdxSource({ id: "appendix", preset: "section-folders", root: "report/appendix" }),
    ]}
  >
    <Sections source="story" />
    <Sections source="appendix" />
  </Press>
  ```
</ApiEntry>

<ApiEntry
  name="mdxSource"
  kind="function"
  importFrom={'import { mdxSource } from "@open-press/core/mdx";'}
  signature={`mdxSource(options: MdxSourceOptions): SourceRegistration`}
  summary="工廠函數，回傳標準化之資料來源註冊物件供 `<Press>` 內部處理。"
>
  <PropsTable
    title="選項 (MdxSourceOptions)"
    rows={[
      {
        name: "id",
        type: "string",
        required: true,
        description: "識別碼。被 `<MdxArea chainId>`、`<Sections source>` 及搜尋/替換 CLI 依賴。",
      },
      {
        name: "preset",
        type: '"section-folders" | "file-list"',
        required: true,
        description: "`section-folders` 採用慣例目錄結構掃描；`file-list` 則依賴顯式提供的 `files` 清單掃描。",
      },
      {
        name: "root",
        type: "string",
        description: "相對於 `press/` 的搜尋根目錄 (如 `report/chapters`)。使用 `file-list` 模式時忽略。",
      },
      {
        name: "files",
        type: "string[]",
        description: "指定 `preset: \"file-list\"` 時的具體 MDX 檔案相對路徑陣列。",
      },
    ]}
  />

  ### 範例：慣例目錄解析 (`section-folders`)

  ```text
  press/
    report/
      chapters/
        01-intro/
          content/
            01-overview.mdx
            02-context.mdx
  ```
  系統會將 `01-intro/content/` 內的檔案依照字典序拼接合併為單一章節區塊流。

  ### 範例：顯式檔案清單 (`file-list`)

  ```tsx
  mdxSource({
    id: "story",
    preset: "file-list",
    files: ["intro.mdx", "results.mdx", "conclusion.mdx"],
  });
  ```
</ApiEntry>

## 內容編譯限制 (Block-only Rule)

來源 MDX 必須嚴格遵循 **僅限區塊 (Block-only)** 規則。由於渲染器需執行精確的區塊測量與分頁，MDX 內之 JSX 標籤必須為頂層區塊元素，嚴禁出現行內 (inline) 混合 JSX 宣告。

### 支援之寫法

```mdx
<TableCaption>Daily ridership</TableCaption>

| Day | Riders |
| --- | --- |
| Mon | 1,204 |
```

### 拒絕之寫法 (將拋出編譯錯誤)

```mdx
這是一段包含 <Highlight>行內標籤</Highlight> 的文字。
```

## 圖表交叉引用

為圖與表設定穩定 ID 後，正文可使用 `@fig-*` 與 `@tbl-*` 引用。OpenPress 會依文件順序把引用轉成目前的圖表編號，並連結至對應目標；新增、刪除或重排圖表時不必改寫正文數字。

```mdx
@fig-system-flow 顯示主要流程，結果整理於 @tbl-results。

<figure id="fig-system-flow">
  <img src="media/system-flow.svg" alt="系統流程" />
  <figcaption>系統主要流程</figcaption>
</figure>

<TableCaption id="tbl-results">評估結果</TableCaption>

| 指標 | 結果 |
| --- | --- |
| MAE | 4.89 |
```

圖的 ID 必須使用 `fig-` 前綴，表格 ID 必須使用 `tbl-` 前綴。ID 在同一個 Press 內不可重複；找不到目標或類型錯誤時，export 會直接失敗。自訂圖表元件應將收到的 `id` 傳給內部的 `<figure>` 或 `<table>`。

## 依賴與系統整合

來源註冊直接解鎖下列系統功能：

- **動態渲染**：`<MdxArea>` 利用 `chainId` 與此來源對接。
- **自動分頁**：`<Sections>` 與 `<Toc>` 依靠來源的目錄層級遍歷執行任務。
- **全域操作**：CLI `search` 與 `replace` 僅作用於經 `mdxSource` 註冊的檔案集合。
- **標記系統**：Workbench 的 `source-edit` 端點與 `@openpress-comment` 標記寫入器依據來源映射表鎖定檔案修改位置。

資源目錄如元件 (`components/`)、媒體 (`media/`) 與主題 (`theme/`) 不涉及分頁渲染機制，因此不在 `sources` 的註冊範圍內，而是由 `<Press>` 其他屬性或慣例處理。
