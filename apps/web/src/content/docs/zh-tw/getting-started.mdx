...
---
<div class="callout">
    <strong>先決條件。</strong> Node.js 20+ 以及 <code>npm</code> 和 <code>npx</code>；framework 開發與 Cloudflare Pages build 建議使用 Node.js 24。
...
```
  ...
  <p>
    ...
    如果要建立 report、proposal、book 或其他 page-based artifact，請走 AI 優先路徑並使用
    <code>openpress-create-pages</code>；create package 只負責建立 workspace shell。
  </p>
...
```
  ...
  <h3>範例：slides/intro/slide.tsx</h3>
...
```
  ...
  <h3>範例：press/`<slug>`/press.tsx (有順序的索引)</h3>
