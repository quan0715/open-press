...
```bash
npm create @open-press@latest <您的專案名稱> -- --type pages
```

如果要在既有 Workspace 新增文件，只建立新的 Press：

```bash
open-press create <slug> --type pages --title "<標題>"
```
...
自動分頁無法表達作者明確的斷頁意圖時，請在 MDX 中把 `<PageBreak />` 放在應從新頁開始的內容前：
...
`PageBreak` 不接受 props 或 children，也不會產生可見內容。不要用空白段落、spacer 或更複雜的 CSS 規則模擬相同效果。
