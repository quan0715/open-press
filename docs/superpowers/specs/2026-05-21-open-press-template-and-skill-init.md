...
> **Naming note**:本 spec 採用新品牌名 **open-press**(取代既有 `qdoc` 標識)。
> - GitHub repo: `quan0715/open-press`
> - CLI bin: `openpress`(`openpress dev` / `openpress build` 等)
> - SKILL: `openpress-init` / `openpress-upgrade`
>
> 既有 codebase 內 `qdoc` 字眼(`engine/cli.mjs` bin、`AGENTS.md` 措辭、SKILL 檔名等)在 framework restructure migration 階段一次 rename(屬獨立 task,非本 spec 範圍)。

**Status**: Draft for review
**Predecessor**: `2026-05-20-qdoc-react-architecture-design.md`(整體架構)
**Supersedes design intent in架構 spec §2 / Appendix B item 1**(原本假設 `@qdoc/core` 走 npm publish — 本份 spec 改走 template + SKILL 模式)
...
原本草稿(已 supersede)是把 engine + runtime 抽成 `@qdoc/core` npm 套件。理由:
...
|---|---|---|
| 新 workspace | `npx @qdoc/core init` | AI 跑 `openpress-init` SKILL |
| 升級 framework | `npm update @qdoc/core` | AI 跑 `openpress-upgrade` SKILL |
| 客製 framework 行為 | fork `@qdoc/core` + 發新版 | `vite.config.ts` alias shadowing |
...
my-doc/
  ...
  document/                     ← user 文件內容(完全擁有)
    ...
    media/                        全域媒體資產
  package.json                  ← user 擁有;scripts 指向 core/cli.mjs;dependencies 不含 @qdoc/*
...
target/
  ...
  .gitignore                     ← node_modules / dist / public/qdoc / .DS_Store
...
- `openpress init` 必須先有 `qdoc` 命令 → 必須先 install framework → 雞生蛋
...
Preset 行為跟原 npm spec 完全相同(MDX loader / inspector / pagination 等),只是 import path 從 `@qdoc/core/vite` 變 `./core/vite-preset.ts`。
...
- **沒有 `@qdoc/core` dep** — framework 是 `./core/` 資料夾
...
從現在的 framework repo(`engine/` + `src/qdoc/` + `document/` 全混在 root)切到 §9 結構:
...
mv src/qdoc/core/* core/primitives/          # BaseX(從 src/qdoc/core/ 移過來)
mv src/qdoc/*.{ts,tsx} core/runtime/         # reader runtime / inspector / hooks
mv src/styles/qdoc/* core/styles/
...
# 4. skills/ 重整
mv skills/qdoc-* skills/openpress-*         # rename
# starter pack 結構整理(若還有舊 layout 殘留)

# 5. Rename qdoc → openpress
# - CLI bin: engine/cli.mjs → core/cli.mjs;package.json bin name: qdoc → openpress
...
| 原架構 spec 位置 | 需要改的內容 |
|---|---|
| §2 三層架構,Layer 1 描述 `@qdoc/core(npm dependency)` | 改成 `core/(workspace 內 framework folder)` |
| §3.1 `document/index.tsx` 範例的 `import { ... } from '@qdoc/core'` | 改成 `import { ... } from './core/primitives'` 或保留 `@qdoc/core` 當 tsconfig alias(指向 `./core/`) |
...
**這份 spec 通過 → 架構 spec 跟一次更新(換掉 `@qdoc/core` 用詞跟相關小段)**。
...
7. ~~Framework repo 名~~ **已決議:`quan0715/open-press`**
   ...
   - Brand: `open-press` 取代 `qdoc`(全 codebase rename 屬 migration task)
...
| Brand name | `open-press`(取代既有 `qdoc`),CLI bin `openpress`,SKILL 命名 `openpress-*` | High |
