...
const resumeTheme = defineDocumentTheme({
  ...
  typography: {
    ...
    body: { font: "body", size: "8.45pt", lineHeight: 1.42, weight: 400, color: "ink", sample: "Built an engineering-led computing company." },
  },
});
...
function ResumePage({ frameKey, chainId, pageIndex, totalPages }: SectionsPageProps) {
  return (
    <Frame frameKey={frameKey} role="manuscript.content" className="reader-page--resume" data-page-index={pageIndex}>
      <div className="page-frame grid h-full min-h-[inherit] w-full grid-rows-[auto_minmax(0,1fr)_auto] bg-[var(--op-theme-color-paper)]" style={{ padding: "11.5mm 14mm" }}>
        <header className="page-header flex items-center justify-between border-b [font-family:var(--op-theme-type-mono-font-family)] uppercase" style={{ borderColor: "var(--op-theme-color-line)", paddingBottom: "2.7mm", color: "var(--op-theme-color-muted)", fontSize: "6.3pt", lineHeight: 1.35, letterSpacing: "0.14em" }}>
          <span>Jensen Huang · NVIDIA</span>
          <span className="text-[var(--op-theme-color-accent)]">Public-source profile</span>
        </header>
        <main className="page-body min-h-0" style={{ paddingBlock: "5mm" }}>
          <MdxArea chainId={chainId} className={RESUME_PROSE_CLASS} style={{ fontFamily: "var(--op-theme-type-body-font-family)", fontSize: "8.45pt", lineHeight: 1.42, color: "var(--op-theme-color-ink)" }} />
        </main>
