...
function FinancialPage({ frameKey, chainId, pageIndex, totalPages, sectionSlug, sectionTitle }: SectionsPageProps) {
  ...
  return (
    <Frame frameKey={frameKey} role="manuscript.content" className={cover ? "reader-page--financial-cover" : "reader-page--financial"}>
      ...
      >
        ...
        >
          <span>{sectionTitle}</span><span>{pageIndex + 1}{totalPages > 1 ? ` / ${totalPages}` : ""}</span>
        </footer>
      </div>
    </Frame>
  );
