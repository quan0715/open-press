...
> **示範元件未隨 workspace 提供完整 chart。** 專案需要時自己在 `document/components/` 下建立。
...
| Component-owned UI | `document/components/ComponentName.tsx` 或 `document/components/ComponentName/index.tsx` | React component + Tailwind / `op-*` semantic class |
| CSS escape hatch | `document/<slug>/theme/*.css` | 僅供舊內容或無法用 Tailwind 表達的瀏覽器修正;新設計優先放 React/Tailwind |
