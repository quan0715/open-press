...
| 規格透明 | User 看到的 Design page 就是 Agent 依循的 design source | 修改風格時先更新 `press/design.md`,再更新 `press/<slug>/theme/` |
...
open-press 初始模板使用字體角色,而不是在每個 selector 裡直接指定字體名稱。`body` 用於正文、表格、caption、註解;`serif` 用於書名、章節標題與正式敘事;`mono` 用於 code、路徑與資料欄位;`display` 是可選的封面大標角色。

UserStory 目前自託管 Latin sans 字體,中文則使用本機繁中字體 fallback。若輸出必須在不同機器上維持完全一致的中文字形,應將授權過的 CJK subset 放入 `press/<slug>/theme/fonts/`,再把該字體加入 `--userstory-font-body` 或 `--userstory-font-serif` 的最前面。

| 用途 | CSS selector / token | 字體 | 大小 | 字重 | 行高 / 字距 | 使用規則 |
| --- | --- | --- | --- | --- | --- | --- |
| Metric display | `.metric-display`, chart figure number | `--userstory-font-body` | `34pt` | 700 | line-height `1`, letter `0` | 只用於資料圖表或單一重點數字 |
| Cover title | `.cover-title` | `--userstory-font-display` | `clamp(36px, 8.5cqw, 64px)` | 300 | line-height `1`, letter `0.01em` | 文件名稱或產品名,整份文件只在封面使用 |
| Cover tagline | `.cover-tagline` | `--userstory-font-serif` | `clamp(14px, 2.4cqw, 20px)` | 300 | letter `0.08em` | 封面一句話定位,不承載長句 |
| Cover subtitle | `.cover-subtitle` | `--userstory-font-body` | `10.5pt-12.5pt` | 400 | line-height `1.55` | 封面主說明,最多一段 |
| Chapter title | `h2` | `--userstory-font-serif` | `14pt-17pt` | 300 | line-height `1.45`, letter `0.04em` | 每個 `##` 產生一個章節 bookmark |
| Section title | `h3` | `--userstory-font-serif` | `11pt-13pt` | 400 | line-height `1.55`, letter `0.03em` | 用於章節內小節,不要過度切碎 |
| Minor heading | `h4` | `--userstory-font-body` | `10pt-11pt` | 500 | muted, letter `0.04em` | 表格前提示或局部標題 |
| Body text | `p`, `li` | `--userstory-font-body` | `9.5pt-10.5pt` | 400 | line-height `1.85` | 長文閱讀基準;不要用過短行距 |
| Table text | `table` | `--userstory-font-body` | `8pt-9pt` | 400 / th 500 | line-height `1.45` | 密集資料,保持掃讀 |
| Caption | `figcaption`, `caption` | `--userstory-font-body` | `7.5pt-8.5pt` | 400 | line-height `1.5`, letter `0.02em` | 圖表下方置中,需可自動編號 |
...
Starter 可以用 React 元件建立視覺對照頁,例如 `press/<slug>/components/TypeSpecimen/`(typography)與 `press/<slug>/components/TokenSwatchGrid/`(color)。
...
Engine 不在 h2/h3 內容前注入任何編號或前綴;export pipeline 會在 build-time 寫入 `data-chapter` / `data-section` attribute(值是 `01`、`1.1` 這種兩位數阿拉伯格式)。實際顯示樣式由 React/Tailwind page component 或 prose preset 決定,這讓不同文件能挑不同 numbering vocabulary 而不動 reader runtime。

預設 prose numbering 樣式:

```tsx
const proseClass = [
  "[&_h2[data-chapter]::before]:content-[attr(data-chapter)]",
  "[&_h3[data-section]::before]:content-[attr(data-section)]",
].join(" ");
```

要換成「一、二、(一)(二)」風格,**不動 engine、不動 pagination**,改 page component 的 numbering formatter 或 prose preset class,不要新增 shared CSS dependency。若需要超出 Tailwind arbitrary variant 能處理的 numbering vocabulary,應該做成 React heading/prose component,在 render 時把 `data-chapter` / `data-section` 轉成可編輯文字或明確的 inline slot。
...
文件實體尺寸由 page geometry token 與 `<Press page>` 共同決定。framework 的 page shell、reader runtime、measurement、PDF print route 都讀同一組值；workspace 不再維護 `base/page-contract.css`。要從 A4 換到 Letter / B5 / 16:9 投影片，優先改 `<Press page>` 或這組 token，其他內容 CSS 不用動。
...
圖片與圖表都要有 caption,並避免孤立在頁尾。一般圖片使用 `figure`;資料視覺化與大段可重用 HTML 都抽成 PascalCase React component,每個 component 是 `press/<slug>/components/ComponentName.tsx` 或 `press/<slug>/components/ComponentName/index.tsx`。

> **示範元件未隨 workspace 提供完整 chart。** 專案需要時自己在 `press/<slug>/components/` 下建立。
...
| 樣式歸樣式 | chart frame、image grid 等視覺 pattern 優先做成 React component 並使用 Tailwind class;只有 Tailwind 無法安全表達的媒體查詢或瀏覽器修正才放局部 CSS |
| 元件歸元件 | 一個視覺 = 一個 `press/<slug>/components/ComponentName/` 包;MDX 只保留 `<ComponentName />` 呼叫 |
...
如果使用者要求新的視覺元件,Agent 應先更新本檔說明,再新增或修改對應的 React component,並優先用 Tailwind / `op-*` semantic class 完成樣式;最後用 reader 預覽檢查 PDF 輸出。
