...
- **`@open-press/cli`** (new package): scaffolder package for OpenPress workspaces. Bundles a framework snapshot in `template/` and copies it into the user workspace; supports style packs, metadata flags, and installs SKILL files under `.claude/skills/` and `.agents/skills/`.
...
- Initial OpenPress workspace and update skills.
...
- Design and style-pack contributor boundaries split by path.
- README's start-in-30-seconds path now points at the CLI scaffolder.

### Removed

- Empty `skills/openpress-rounddev/` placeholder.
- Writing skill registry reference; priority list inlined into the writing workflow.
...
- Design and style-pack contributor boundaries split by path (`document/theme/` vs `skills/<pack>/starter/document/theme/`) instead of by topic.
- The writing workflow owns the `<TableCaption>` placement rule; style packs no longer redefine it.
...
- Bundled OpenPress agent skills.
...
- CLI starter pack initialization.
...
- Test suite consolidated around framework-level pipeline and unit tests.
