...
Do not create `slides/<id>/style.css`, `layouts/*.css`, `ui/*.css`, or extra `theme/*.css` by default. Slide styling should use the shared Tailwind `op-*` semantic layer plus the active Press theme. Local CSS is an escape hatch only after a user explicitly asks for custom CSS that cannot be represented with template composition and semantic classes.
