...
open-press 本機 workbench 是 User 和 Agent 共同工作的預設入口。Agent 透過 `qdoc` skill 打開本機 workbench，User 在瀏覽器裡檢查兩個視角：Document 看正式文件、Project 看來源與素材狀態。風格規則改在 `document/design.md`，編輯器或 GitHub 上直接讀。
...
文件可交付前，Agent 應先跑 `qdoc:export`、`qdoc:validate`，必要時再跑 `qdoc:pdf`。若要公開上線，改用 `openpress-deploy` 進入部署流程；部署前必須確認 target project、公開網址、授權素材與文件中的事實。
