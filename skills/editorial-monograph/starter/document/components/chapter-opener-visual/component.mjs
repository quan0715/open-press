...
export function render({ attrs, helpers }) {
  const variant = attrs.data || attrs.variant || "linked-list";
  if (!Object.hasOwn(VISUALS, variant)) {
    throw new Error(`chapter-opener-visual unknown variant: ${variant}`);
  }

  const tone = TONES.has(attrs.tone) ? attrs.tone : "sage";
  const className = helpers.classNames(
    "chapter-opener-illustration",
    `chapter-opener-illustration--${variant}`,
    `chapter-opener-tone--${tone}`,
  );

  return `<figure class="${helpers.escapeAttr(className)}" aria-hidden="true">
    ${VISUALS[variant]}
  </figure>`;
}
