...
| 建置輸出 | `npm run build` | 結構驗證 + 渲染 `dist-react/`，並寫入 `public/openpress/workspace.json` 與每個 Press 的 `document.json` |
...
open-press 的部署設定放在 `package.json` 的 `"openpress"` 欄位，而不是藏在前端按鈕裡。以 Cloudflare Pages 為例，workspace 需要明確的 adapter、輸出目錄與 project name：
