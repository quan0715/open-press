...
2. **拷貝 starter 檔案**：
   ...
   skills/editorial-monograph/starter/openpress.config.mjs → <workspace root>/openpress.config.mjs（新工作區）
   ```

3. **填入 metadata**：詢問 user `title` / `subtitle` / `organization`，寫入 `document/index.tsx` 與 `document/openpress.config.mjs`。
...
>   新工作區拷貝 `starter/openpress.config.mjs` → workspace root、
>   詢問 user 填 title / subtitle / organization、
>   跑 `openpress:validate` 確認。
