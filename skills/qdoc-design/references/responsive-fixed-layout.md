...
Use this reference when editing QDoc styles that affect mobile, tablet, zoom, spread mode, or responsive behavior.

## Core Principle

QDoc pages have canonical page geometry. Responsive behavior should scale the page and surrounding workspace, not re-author the document for every viewport.

A deployed QDoc reader should stay DOM-rendered and QDoc-owned. Do not embed the browser PDF viewer to solve responsive problems; that adds a second reader UI and breaks the intended workspace model.
...
- The React QDoc workbench shell may be responsive: side panels can collapse, navigation can become denser, and controls may hide secondary metadata.
...
- There is no embedded browser PDF toolbar inside the QDoc reader.
